...
const onecliSecretGrantMemo = new Set<string>();
...
export function buildMounts(
  ...
  // A Feishu DM container receives exactly one user's own CLI config and key
  // store at the official default paths. Non-DM sessions receive the service
  // bot profile read-only so group/public workflows can use app permissions
  // without ever seeing a user's OAuth token. The guarded `lark-cli-user`
  // wrapper still requires the per-user marker and therefore remains DM-only.
  const larkCliUser = resolveSessionFeishuDmUser(agentGroup.id, session);
  if (larkCliUser) {
    const userHome = ensureLarkCliUserHome(larkCliUser);
    mounts.push({
      hostPath: path.join(userHome, '.lark-cli'),
      containerPath: '/home/node/.lark-cli',
      readonly: false,
    });
    mounts.push({
      hostPath: path.join(userHome, '.local', 'share', 'lark-cli'),
      containerPath: '/home/node/.local/share/lark-cli',
      readonly: false,
    });
  } else {
    const botHome = larkCliBotHome();
    if (botHome) {
      mounts.push({
        hostPath: path.join(botHome, '.lark-cli'),
        containerPath: '/home/node/.lark-cli',
        readonly: true,
      });
      mounts.push({
        hostPath: path.join(botHome, '.local', 'share', 'lark-cli'),
        containerPath: '/home/node/.local/share/lark-cli',
        readonly: true,
      });
    }
...
export function extractFeishuSenderId(content: string): string | null {
  try {
    ...
    const raw =
      ...
      parsed.author !== null &&
      typeof (parsed.author as Record<string, unknown>).userId === 'string'
        ? ((parsed.author as Record<string, unknown>).userId as string)
        : null);
    if (!raw) return null;
    return raw.startsWith('fs:') ? raw : `fs:${raw}`;
  } catch {
    return null;
  }
}

export function resolveSessionFeishuDmUser(agentGroupId: string, session: Session): string | null {
  if (!session.messaging_group_id) return null;
  const messagingGroup = getMessagingGroup(session.messaging_group_id);
  if (!messagingGroup || messagingGroup.channel_type !== 'fs' || messagingGroup.is_group !== 0) return null;
  ...
  try {
    ...
    try {
      ...
      return row ? extractFeishuSenderId(row.content) : null;
    } finally {
      db.close();
    }
  } catch (err) {
    log.warn('Unable to resolve Feishu DM user for isolated lark-cli mount', {
...
function ensureLarkCliUserHome(userId: string): string {
  const root = path.join(DATA_DIR, 'lark-cli-users');
  fs.mkdirSync(root, { recursive: true, mode: 0o700 });
  const dir = path.join(root, userId.replace(/[^A-Za-z0-9_.-]/g, '_'));
  fs.mkdirSync(path.join(dir, '.lark-cli'), { recursive: true, mode: 0o700 });
  fs.mkdirSync(path.join(dir, '.local', 'share', 'lark-cli'), { recursive: true, mode: 0o700 });
  fs.writeFileSync(path.join(dir, '.lark-cli', '.nanoclaw-user-id'), `${userId}\n`, { mode: 0o600 });
  return dir;
}

function larkCliBotHome(): string | null {
  const dir = path.join(DATA_DIR, 'lark-cli-home');
  const configDir = path.join(dir, '.lark-cli');
  const storeDir = path.join(dir, '.local', 'share', 'lark-cli');
  return fs.existsSync(configDir) && fs.existsSync(storeDir) ? dir : null;
}
...
export function syncSkillSymlinks(
  ...
  for (const skill of desired) {
    ...
    const desiredTarget = `/app/skills/${skill}`;
...
async function buildContainerArgs(
  ...
  args.push('-e', 'PATH=/usr/local/bin:/pnpm:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin');
  ...
  const larkCliWrapper = path.join(process.cwd(), 'container', 'bin', 'lark-cli');
  if (fs.existsSync(larkCliWrapper)) {
    args.push(...readonlyMountArgs(larkCliWrapper, '/usr/local/bin/lark-cli'));
  }
...
async function ensureModelSecretGrant(agentIdentifier: string, contributedBaseUrl?: string): Promise<void> {
  ...
  try {
    const [agents, secrets] = await Promise.all([
      onecliJson<OneCliAgentRecord[]>('/v1/agents'),
      onecliJson<OneCliSecretRecord[]>('/v1/secrets'),
    ]);
    const agent = agents.find((row) => row.identifier === agentIdentifier);
    const matchingSecrets = secrets.filter((row) => row.hostPattern && hostMatchesPattern(host, row.hostPattern));
    if (!agent || matchingSecrets.length === 0) return;
    ...
    for (const secret of matchingSecrets) {
      ...
      log.info('OneCLI model secret grant ensured', { agentIdentifier, host, secretId: secret.id });
    }
  } catch (err) {
...
async function onecliJson<T>(pathName: string): Promise<T> {
  const response = await fetch(`${trimOneCliUrl()}${pathName}`, {
    headers: onecliHeaders(),
    signal: AbortSignal.timeout(5000),
  });
  if (!response.ok) {
    throw new Error(`OneCLI ${pathName} returned ${response.status}`);
  }
  return (await response.json()) as T;
}

function trimOneCliUrl(): string {
  return ONECLI_URL.replace(/\/+$/, '');
}

function onecliHeaders(): Record<string, string> {
  return {
