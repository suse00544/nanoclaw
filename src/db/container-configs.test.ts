...
describe('ensureContainerConfig default stamping', () => {
  beforeEach(() => {
    const db = initTestDb();
    runMigrations(db);
  });
  ...
  it('stamps the instance model only on a fresh row', () => {
    makeGroup('ag-model');
    ensureContainerConfig('ag-model');
    expect(getContainerConfig('ag-model')?.model).toBe(DEFAULT_AGENT_MODEL);

    updateContainerConfigScalars('ag-model', { model: 'custom-model' });
    ensureContainerConfig('ag-model');
    expect(getContainerConfig('ag-model')?.model).toBe('custom-model');
  });
