...
/**
 * Lookup for platforms where a cold-DM handle (user id / open id) differs
 * from the actual DM conversation id, but the inbound DM row has already
 * taught us that mapping. Feishu p2p chats are the motivating case: inbound
 * messages arrive on an `oc_...` chat id, while users are keyed by `ou_...`.
 */
export function getMessagingGroupByDmHandle(
  channelType: string,
  handle: string,
  instance?: string,
): MessagingGroup | undefined {
  if (instance !== undefined) {
    return getDb()
      .prepare(
        `SELECT * FROM messaging_groups
          WHERE channel_type = ? AND instance = ? AND is_group = 0 AND name = ?
       ORDER BY created_at DESC
          LIMIT 1`,
      )
      .get(channelType, instance, handle) as MessagingGroup | undefined;
  }
  return getDb()
    .prepare(
      `SELECT * FROM messaging_groups
        WHERE channel_type = ? AND is_group = 0 AND name = ?
     ORDER BY (instance = channel_type) DESC, created_at DESC
        LIMIT 1`,
    )
    .get(channelType, handle) as MessagingGroup | undefined;
}
  ...
  getDb()
    .prepare(
       `INSERT INTO messaging_group_agents (
         ...
         session_mode, priority, threads, created_at
       )
       VALUES (
         @id, @messaging_group_id, @agent_group_id,
         @engage_mode, @engage_pattern, @sender_scope, @ignored_message_policy,
    ...
    .run({ ...mga, threads: mga.threads ?? null });
