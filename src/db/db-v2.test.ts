...
describe('migrations', () => {
  ...
  it('persists question card bodies for terminal rendering (022)', () => {
    const db = initTestDb();
    runMigrations(db);
    const col = db
      .prepare(`SELECT type, "notnull", dflt_value FROM pragma_table_info('pending_questions') WHERE name = 'question'`)
      .get() as { type: string; notnull: number; dflt_value: string } | undefined;
    expect(col).toEqual({ type: 'TEXT', notnull: 1, dflt_value: "''" });
  });
});
