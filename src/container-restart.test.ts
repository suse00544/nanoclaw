...
function makeSession(
  ...
  agentGroupId: string,
  status = 'active',
  messagingGroupId: string | null = null,
  threadId: string | null = null,
) {
  return { id, agent_group_id: agentGroupId, status, messaging_group_id: messagingGroupId, thread_id: threadId };
}
...
describe('restartAgentGroupContainers', () => {
  ...
  it('routes wake messages through the session messaging group when available', () => {
    mockGetSessionsByAgentGroup.mockReturnValue([makeSession('s1', 'g1', 'active', 'mg1', 'thread-1')]);
    mockGetMessagingGroup.mockReturnValue({
      id: 'mg1',
      channel_type: 'fs',
      platform_id: 'fs:oc_group',
    });
    mockIsContainerRunning.mockReturnValue(true);
    ...
    expect(mockGetMessagingGroup).toHaveBeenCalledWith('mg1');
    expect(msg.platformId).toBe('fs:oc_group');
    expect(msg.channelType).toBe('fs');
