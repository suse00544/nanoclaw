...
export async function ensureUserDm(userId: string): Promise<MessagingGroup | null> {
  ...
  // Prefer a real inbound DM row when it exists. Some platforms expose a
  // stable user handle to identify people, while delivery should target a
  // separate p2p conversation id already learned from inbound traffic.
  const now = new Date().toISOString();
  const inboundDm = getMessagingGroupByDmHandle(channelType, handle);
  if (inboundDm) {
    upsertUserDm({
      user_id: userId,
      channel_type: channelType,
      messaging_group_id: inboundDm.id,
      resolved_at: now,
    });
    return inboundDm;
  }
