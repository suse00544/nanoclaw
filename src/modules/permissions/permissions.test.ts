...
describe('ensureUserDm', () => {
  ...
  it('reuses an inbound p2p DM row keyed by user handle before opening a cold DM', async () => {
    const mock = await mountMockAdapter('fs', async (handle) => handle);
    seedUser('fs:ou-user-1', 'fs');
    createMessagingGroup({
      id: 'mg-feishu-p2p',
      channel_type: 'fs',
      platform_id: 'fs:oc-p2p-chat',
      instance: 'fs',
      name: 'ou-user-1',
      is_group: 0,
    ...
    const mg = await ensureUserDm('fs:ou-user-1');

    expect(mg?.id).toBe('mg-feishu-p2p');
    expect(mg?.platform_id).toBe('fs:oc-p2p-chat');
    expect(mock.openDMCalls).toEqual([]);
    expect(getUserDm('fs:ou-user-1', 'fs')?.messaging_group_id).toBe('mg-feishu-p2p');
  });
});
