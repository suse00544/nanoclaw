...
function buildApprovalOptions(agentGroups: AgentGroup[], approverUserId?: string | null): RawOption[] {
  ...
  if (visibleAgentGroups.length === 1) {
    options.push({
      label: `绑定到 ${visibleAgentGroups[0].name}`,
      selectedLabel: `✅ 已绑定到 ${visibleAgentGroups[0].name}`,
    ...
    options.push({
      label: '绑定已有 agent',
      selectedLabel: '📋 正在选择 agent...',
      value: CHOOSE_EXISTING_VALUE,
    });
  }
  options.push({
    label: '新建独立 agent',
    selectedLabel: '🆕 正在新建 agent...',
...
function buildQuestionText(
  ...
  const who = senderName ?? '有人';
  const note = ruleNote ? ` 绑定后，agent ${ruleNote}。` : '';
  if (isGroup) {
    const where = channelName ? `${channelName}（${channelType}）` : `一个 ${channelType} 群`;
    return `${who} 在 ${where} 里 @ 了机器人。${note}请选择这个群要绑定到哪个 agent。`;
  }
  return `${who} 在 ${channelType} 给机器人发了私聊。${note}请选择这段私聊要绑定到哪个 agent。`;
}
...
function describeResolvedRule(
  ...
  try {
    ...
    if (engage.engage_mode !== 'pattern') {
      return isGroup ? '只会回复群里的 @ 消息' : '会回复 @ 消息';
    }
    return engage.engage_pattern === '.'
      ? '会回复所有消息'
      : `会回复匹配 ${engage.engage_pattern} 的消息`;
  } catch {
    return null;
  }
}
...
export async function requestChannelApproval(input: RequestChannelApprovalInput): Promise<void> {
  ...
  const title = isGroup ? '📣 新群里有人 @ 了机器人' : '💬 新的机器人私聊';
  ...
  try {
    ...
    if (!platformMsgId) {
      log.warn('Channel registration card delivery returned no platform message id', {
        messagingGroupId,
        deliveryChannelType: delivery.messagingGroup.channel_type,
        deliveryPlatformId: delivery.messagingGroup.platform_id,
