...
// (mention groups, pattern '.' DMs). Without it, the no-live-adapter
...
registerChannelAdapter('fs', { factory: () => null, defaults: telegramDefaults });
...
vi.mock('../../config.js', async () => {
  ...
  return {
    ...
    DEFAULT_GROUP_AGENT_GROUP_ID: 'ag-1',
  };
});
...
function feishuGroupMention(platformId: string, text = '@Beacon hello') {
  const base = groupMention(platformId, text);
  return {
    ...base,
    channelType: 'fs',
    platformId,
    ...
    message: {
      ...base.message,
      content: JSON.stringify({ senderId: 'ou-caller', senderName: 'Caller', text }),
    },
  };
}

function feishuDmEvent(platformId: string, text = 'hello') {
  return {
    channelType: 'fs',
    platformId,
    instance: 'fs',
...
describe('unknown-channel registration flow', () => {
  it('auto-wires new Feishu groups to the configured shared agent group without a card', async () => {
    ...
    await routeInbound(feishuGroupMention('fs:oc-auto'));
    ...
    const mg = getMessagingGroupByPlatform('fs', 'fs:oc-auto', 'fs');
    expect(mg).toBeDefined();
    const wiring = getDb()
      .prepare('SELECT * FROM messaging_group_agents WHERE messaging_group_id = ? AND agent_group_id = ?')
      .get(mg!.id, 'ag-1') as
      | { sender_scope: string; ignored_message_policy: string; session_mode: string; threads: number | null }
      | undefined;
    expect(wiring).toBeDefined();
    expect(wiring!.sender_scope).toBe('all');
    expect(wiring!.ignored_message_policy).toBe('accumulate');
    expect(wiring!.session_mode).toBe('shared');
    expect(wiring!.threads).toBe(0);
    const member = getDb()
      .prepare('SELECT 1 AS x FROM agent_group_members WHERE user_id = ? AND agent_group_id = ?')
      .get('fs:ou-caller', 'ag-1');
    expect(member).toBeDefined();
    const admin = getDb()
      .prepare('SELECT 1 AS x FROM user_roles WHERE user_id = ? AND role = ? AND agent_group_id = ?')
      .get('fs:ou-caller', 'admin', 'ag-1');
    expect(admin).toBeUndefined();

    const session = getDb()
      .prepare('SELECT id FROM sessions WHERE agent_group_id = ? AND messaging_group_id = ?')
      .get('ag-1', mg!.id) as { id: string };
    const { withInboundDb } = await import('../../session-manager.js');
    const inboundText = withInboundDb('ag-1', session.id, (db) => {
      const row = db.prepare("SELECT content FROM messages_in WHERE kind = 'chat' ORDER BY seq LIMIT 1").get() as {
        content: string;
      };
      return (JSON.parse(row.content) as { text: string }).text;
    });
    expect(inboundText).toBe('@Beacon hello');
    expect(wakeContainer).toHaveBeenCalledTimes(1);
  });

  it('auto-creates personal agent groups for new Feishu DMs', async () => {
    const { routeInbound } = await import('../../router.js');
    const { getDb } = await import('../../db/connection.js');
    const { wakeContainer } = await import('../../container-runner.js');
    (wakeContainer as unknown as ReturnType<typeof vi.fn>).mockClear();

    await routeInbound(feishuDmEvent('fs:ou-new-dm'));
    await new Promise((r) => setTimeout(r, 20));

    expect(deliverMock).not.toHaveBeenCalled();
    const agent = getDb().prepare("SELECT id, name FROM agent_groups WHERE name LIKE '%agent' AND id <> 'ag-1'").get() as
      | { id: string; name: string }
      | undefined;
    expect(agent).toBeDefined();
    const containerConfig = getDb()
      .prepare('SELECT assistant_name FROM container_configs WHERE agent_group_id = ?')
      .get(agent!.id) as { assistant_name: string | null };
    expect(containerConfig.assistant_name).toBe(ASSISTANT_NAME);

    const mg = getMessagingGroupByPlatform('fs', 'fs:ou-new-dm', 'fs');
    expect(mg).toBeDefined();
    const wiring = getDb()
      .prepare('SELECT * FROM messaging_group_agents WHERE messaging_group_id = ? AND agent_group_id = ?')
      .get(mg!.id, agent!.id);
    ...
    const admin = getDb()
      .prepare('SELECT 1 AS x FROM user_roles WHERE user_id = ? AND role = ? AND agent_group_id = ?')
      .get('fs:ou-caller', 'admin', agent!.id);
    expect(admin).toBeDefined();

    const session = getDb()
      .prepare('SELECT id FROM sessions WHERE agent_group_id = ? AND messaging_group_id = ?')
      .get(agent!.id, mg!.id) as { id: string };
    const { withInboundDb } = await import('../../session-manager.js');
    const inboundText = withInboundDb(agent!.id, session.id, (db) => {
      const row = db.prepare("SELECT content FROM messages_in WHERE kind = 'chat' ORDER BY seq LIMIT 1").get() as {
        content: string;
      };
      return (JSON.parse(row.content) as { text: string }).text;
    });
    expect(inboundText).toContain('first conversation with a newly connected user');
    expect(inboundText).toContain("User's first message: hello");
    expect(wakeContainer).toHaveBeenCalledTimes(1);
  });
  ...
  it('delivers an approval card on mention into an unwired group', async () => {
    ...
    expect(payload.question).toContain('只会回复群里的 @ 消息');
    // Single-agent card offers a direct bind button.
  ...
  it('delivers a card on DM too (non-threaded event)', async () => {
    ...
    expect(payload.question).toContain('会回复所有消息');
    const { getDb } = await import('../../db/connection.js');
  ...
  it('approve → creates wiring, admits triggering sender, replays', async () => {
    ...
    expect(mga.engage_mode).toBe('mention'); // declared group default
    ...
    // Triggering sender is still auto-admitted as a member for future explicit
    // per-agent access checks, even though group wirings allow all senders.
    const member = getDb()
      .prepare('SELECT 1 AS x FROM agent_group_members WHERE user_id = ? AND agent_group_id = ?')
