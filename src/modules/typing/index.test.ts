...
describe('stopTypingRefresh — platform cleanup', () => {
  it('clears a platform-owned typing marker even when no reply was delivered', async () => {
    const cleared: Call[] = [];
    setTypingAdapter({
      async setTyping() {},
    ...
    startTypingRefresh('sess-1', 'ag-1', 'fs', 'fs:oc_chat', null, 'feishu');
    await vi.advanceTimersByTimeAsync(0);
