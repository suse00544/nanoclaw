...
- In an interactive chat, return the greeting as your normal final response. Do not also call `send_message`; doing both sends the same onboarding message twice. Use an explicit messaging tool only when onboarding runs as a background task with no normal reply channel.
