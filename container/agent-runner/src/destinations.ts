...
function buildDestinationsSection(mode: SessionMode): string {
  ...
  lines.push('');
  lines.push(
    'For useful follow-up reply chips, put 1-3 short options at the end of that same block inside `<suggestions>`, one option per bullet line. Use this sparingly, only when the user has a clear next step or likely reply choice; each option should read like natural user text, be concise, and not repeat the message body. Example: `<suggestions>\\n- 继续\\n- 给我例子\\n</suggestions>`. Supported channels may show these as tappable suggestions; unsupported channels ignore them.',
  );
  lines.push('');
  lines.push(
