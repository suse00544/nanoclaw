...
describe('poll loop integration', () => {
  ...
  it('anchors destination replies to the latest triggering row, not later context-only rows', async () => {
    insertMessage('m-context-before', { sender: 'Alice', text: 'context before' }, {
      platformId: 'chan-1',
      channelType: 'discord',
      threadId: 'thread-context-before',
      trigger: 0,
    });
    insertMessage('m-trigger', { sender: 'Alice', text: '@bot wake up' }, {
      platformId: 'chan-1',
      channelType: 'discord',
      threadId: 'thread-trigger',
      trigger: 1,
    });
    insertMessage('m-context-after', { sender: 'Alice', text: 'context after' }, {
      platformId: 'chan-1',
      channelType: 'discord',
      threadId: 'thread-context-after',
      trigger: 0,
    });
    ...
    const out = getUndeliveredMessages();
    expect(out).toHaveLength(1);
    expect(out[0].thread_id).toBe('thread-trigger');
    expect(out[0].in_reply_to).toBe('m-trigger');
