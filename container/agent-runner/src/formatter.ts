 ...
 * Uses the newest trigger=1 message as the visible reply anchor; accumulated
 * trigger=0 rows are context only and must not steal the reply target.
 */
export function extractRouting(messages: MessageInRow[]): RoutingContext {
  const first = [...messages].reverse().find((m) => m.trigger === 1) ?? messages[0];
  return {
