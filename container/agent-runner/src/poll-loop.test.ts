...
function insertRoutedMessage(
  ...
  getInboundDb()
    ...
    .run(id, kind, opts.platformId, opts.channelType, opts.threadId, opts.trigger ?? 1, JSON.stringify(content));
}
...
describe('accumulate gate (trigger column)', () => {
  ...
  it('does not let newer context-only rows starve the latest trigger', () => {
    insertMessage('m-trigger', 'chat', { sender: 'B', text: '@bot wake up' }, { trigger: 1 });
    for (let i = 0; i < 20; i++) {
      insertMessage(`m-context-${i}`, 'chat', { sender: 'A', text: `context ${i}` }, { trigger: 0 });
    }

    const messages = getPendingMessages();
    expect(messages.some((m) => m.id === 'm-trigger')).toBe(true);
    expect(messages.some((m) => m.trigger === 1)).toBe(true);
  });
...
describe('routing', () => {
  ...
  it('anchors routing on the newest trigger row, not accumulated context', () => {
    getInboundDb()
      .prepare(
        `INSERT INTO messages_in (id, kind, timestamp, status, platform_id, channel_type, thread_id, trigger, content)
       VALUES
         ('m-context', 'chat', datetime('now'), 'pending', 'chan-123', 'discord', 'thread-context', 0, '{"text":"context"}'),
         ('m-trigger', 'chat', datetime('now'), 'pending', 'chan-123', 'discord', 'thread-trigger', 1, '{"text":"@bot"}')`,
      )
      .run();

    const routing = extractRouting(getPendingMessages());
    expect(routing.threadId).toBe('thread-trigger');
    expect(routing.inReplyTo).toBe('m-trigger');
  });
});
...
it('aborts an active task query when a chat message arrives and leaves the chat pending', async () => {
  const pushes: string[] = [];
  let aborted = false;

  async function* events(): AsyncGenerator<ProviderEvent> {
    yield { type: 'init', continuation: 'sess-1' };
    insertMessage('chat-1', 'chat', { sender: 'Tim', text: '@Beacon reply to me' });
  ...
  expect(pushes).toHaveLength(0);
  expect(getPendingMessages().map((m) => m.id)).toEqual(['chat-1']);
});

it('keeps a pushed follow-up processing until its provider result arrives', async () => {
  const pushes: string[] = [];
  let statusBeforeResult: string | undefined;

  async function* events(): AsyncGenerator<ProviderEvent> {
    yield { type: 'init', continuation: 'sess-1' };
    yield { type: 'result', text: null };
    insertMessage('chat-1', 'chat', { sender: 'Tim', text: 'second turn' });
    const deadline = Date.now() + 2000;
    while (pushes.length === 0 && Date.now() < deadline) {
      await new Promise((resolve) => setTimeout(resolve, 25));
    }
    statusBeforeResult = (
      getOutboundDb().prepare('SELECT status FROM processing_ack WHERE message_id = ?').get('chat-1') as
        | { status: string }
        | undefined
    )?.status;
    yield { type: 'result', text: null };
  ...
  expect(statusBeforeResult).toBe('processing');
  const finalAck = getOutboundDb()
    .prepare('SELECT status FROM processing_ack WHERE message_id = ?')
    .get('chat-1') as { status: string };
  expect(finalAck.status).toBe('completed');
});

it('routes pushed follow-up results to the follow-up trigger thread', async () => {
  getInboundDb()
    .prepare(
      `INSERT INTO destinations (name, display_name, type, channel_type, platform_id, agent_group_id)
  ...
  async function* events(): AsyncGenerator<ProviderEvent> {
    yield { type: 'init', continuation: 'sess-1' };
    yield { type: 'result', text: '<message to="discord-test">first</message>' };
    insertRoutedMessage('m-follow', 'chat', { sender: 'Tim', text: '@Beacon second' }, {
      platformId: 'chan-1',
      channelType: 'discord',
    ...
    yield { type: 'result', text: '<message to="discord-test">second</message>' };
  }
  ...
  expect(out[0].thread_id).toBe('thread-initial');
  expect(out[0].in_reply_to).toBe('m-initial');
  expect(out[1].thread_id).toBe('thread-follow');
  expect(out[1].in_reply_to).toBe('m-follow');
});

it('ends the provider stream after all successful exchanges complete', async () => {
  let ended = false;
  ...
  async function* events(): AsyncGenerator<ProviderEvent> {
    ...
    if (!ended) throw new Error('provider stream was not ended');
  }
  ...
  ).resolves.toEqual({ continuation: 'sess-1' });

  expect(ended).toBe(true);
});

it('aborts a provider stream after the query idle timeout', async () => {
  let aborted = false;
  ...
  ).rejects.toThrow('Provider idle timeout after 1s without activity');
