...
describe('compact_boundary translation', () => {
  ...
  it('defaults auto-compaction to a 1M-token window', async () => {
    sdkMessages.length = 0;
    lastQueryOptions = undefined;
    ...
    for await (const _e of q.events) {
      // Drain the mocked SDK stream so options are captured.
    }

    const args = lastQueryOptions as { options?: { env?: Record<string, string> } };
    expect(args.options?.env?.CLAUDE_CODE_AUTO_COMPACT_WINDOW).toBe('1000000');
  });
});
