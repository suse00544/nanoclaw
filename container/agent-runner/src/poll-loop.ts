...
const DEFAULT_QUERY_IDLE_TIMEOUT_MS = 180_000;
...
function queryIdleTimeoutMs(): number {
  const raw = process.env.NANOCLAW_QUERY_IDLE_TIMEOUT_MS;
  if (!raw) return DEFAULT_QUERY_IDLE_TIMEOUT_MS;
  const parsed = Number(raw);
  return Number.isFinite(parsed) && parsed >= 0 ? parsed : DEFAULT_QUERY_IDLE_TIMEOUT_MS;
}
...
export async function processQuery(
  ...
  idleTimeoutMs = queryIdleTimeoutMs(),
): Promise<QueryResult> {
  ...
  const archiveMessageIds: string[][] = [initialBatchIds];
  const archiveRoutings: RoutingContext[] = [routing];

  const completeCurrentExchange = (): void => {
    markCompleted(archiveMessageIds.shift() ?? []);
    archivePrompts.shift();
    archiveRoutings.shift();
  };
  ...
  let normalEndHandle: ReturnType<typeof setTimeout> | undefined;
  let corruptionStreak = 0;
  const pollHandle = setInterval(() => {
    if (done || pollInFlight || endedForCommand || endingNormally) return;
    pollInFlight = true;
    ...
    void (async () => {
      try {
        ...
        // Task runs and interactive turns have different delivery contracts.
        // In particular, task output is log-only unless sent through an
        // explicit tool, while chat output is delivered to the triggering
        // channel. Never push one kind into an active query of the other kind:
        // abort the stream and leave the new rows pending for the outer loop to
        // process with fresh routing.
        if (newMessages.some((m) => (m.kind === 'task') !== routing.taskRun)) {
          log(`Pending ${routing.taskRun ? 'interactive' : 'task'} message — aborting active stream for fresh routing`);
          endedForCommand = true;
          query.abort();
          return;
        }
        ...
        if (normalEndHandle) {
          clearTimeout(normalEndHandle);
          normalEndHandle = undefined;
        }
        unwrappedNudged = false;
        taskBlockNudged = false;
        query.push(prompt);
        archivePrompts.push(prompt);
        archiveMessageIds.push(keptIds);
        archiveRoutings.push(followUpRouting);
      } catch (err) {
  ...
  const scheduleEndQueryIfNoPendingExchanges = (): void => {
    if (endingNormally || normalEndHandle || archivePrompts.length > 0) return;
    normalEndHandle = setTimeout(() => {
      normalEndHandle = undefined;
      if (endingNormally || archivePrompts.length > 0) return;
      endingNormally = true;
      query.end();
    }, ACTIVE_POLL_INTERVAL_MS * 2);
  };

  try {
    for await (const event of withQueryIdleTimeout(query, idleTimeoutMs)) {
      const eventRouting = archiveRoutings[0] ?? routing;
      ...
      if (event.type === 'init') {
        ...
        if (event.text) {
          const { sent, hasUnwrapped, taskBlocks } = dispatchResultText(event.text, eventRouting);
          const willRetryTaskBlocks = shouldNudgeTaskBlocks(eventRouting.taskRun, taskBlocks, taskBlockNudged);
          ...
          // second run summary.
          if (eventRouting.taskRun && !taskBlockNudged) autoAppendTaskLog(event.text);
          if (sent === 0 && event.isError === true && !eventRouting.taskRun) {
            // Non-retryable error turn (e.g. a 403 billing_error) with no
            // <message> envelope: deliver the notice instead of dropping it as
            // scratchpad, and skip the re-wrap nudge — it would just re-hammer
            // the failing gateway turn after turn.
            deliverErrorResult(event.text, eventRouting);
            ...
            if (!willRetryWrapping && !willRetryTaskBlocks) {
              completeCurrentExchange();
              scheduleEndQueryIfNoPendingExchanges();
            }
          }
        } else {
          completeCurrentExchange();
    ...
    clearInterval(pollHandle);
    if (normalEndHandle) clearTimeout(normalEndHandle);
  }

  return { continuation: queryContinuation };
}

async function* withQueryIdleTimeout(query: AgentQuery, timeoutMs: number): AsyncGenerator<ProviderEvent> {
  if (timeoutMs <= 0) {
    for await (const event of query.events) yield event;
    return;
  }
  ...
  try {
    while (true) {
      ...
      const timeout = new Promise<IteratorResult<ProviderEvent>>((_, reject) => {
        timeoutHandle = setTimeout(() => {
          query.abort();
          reject(new Error(`Provider idle timeout after ${Math.ceil(timeoutMs / 1000)}s without activity`));
        }, timeoutMs);
      });
...
export function dispatchResultText(
  ...
  while ((match = MESSAGE_RE.exec(text)) !== null) {
    ...
    const { body, suggestions } = parseMessageBody(match[2]);
    ...
    sendToDestination(dest, body, routing, suggestions);
    sent++;
  }
...
export function parseMessageBody(rawBody: string): ParsedMessageBody {
  let body = rawBody;
  const suggestions: string[] = [];
  body = body.replace(/<suggestions\s*>([\s\S]*?)<\/suggestions>/gi, (_match, rawSuggestions: string) => {
    suggestions.push(...parseSuggestions(rawSuggestions));
    return '';
  });
  return {
...
function sendToDestination(dest: DestinationEntry, body: string, routing: RoutingContext, suggestions: string[] = []): void {
  ...
  if (channelType === routing.channelType && platformId === routing.platformId) {
    writeMessageOut({
      ...
      thread_id: routing.threadId,
      content: JSON.stringify({ text: body, ...(suggestions.length > 0 ? { suggestions } : {}) }),
    });
    return;
  }
  // Resolve thread_id per-destination from the most recent triggering inbound
  // message that came from this same channel+platform. In agent-shared
  // sessions, different destinations have different thread contexts — using a
  // single routing.threadId would stamp one channel's thread onto another.
  // Prefer trigger=1 rows so accumulated context-only messages don't become
  // the visible reply anchor for a response caused by an earlier mention.
...
/**
 * Find the thread_id and message id from the most recent triggering inbound
 * message matching the given channel+platform. Falls back to the latest row
 * only for legacy rows or non-chat paths that have no trigger=1 anchor.
 */
    ...
    const row = db
      .prepare(
        `SELECT thread_id, id FROM messages_in
         ...
         ORDER BY CASE WHEN trigger = 1 THEN 0 ELSE 1 END, COALESCE(seq, rowid) DESC LIMIT 1`,
      )
